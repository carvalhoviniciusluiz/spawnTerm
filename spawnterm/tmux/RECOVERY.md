...
2. The **tmux server keeps running.** Every agent spawned with
   `spawnterm-tmux spawn` (flag ON) is a process inside a tmux session named
...
spawnterm/tmux/spawnterm-tmux attach --role worker --task "build #5"
# …or by explicit id / session name:
spawnterm/tmux/spawnterm-tmux attach --id 5
spawnterm/tmux/spawnterm-tmux attach --session st-worker-build-5
```
...
`spawn` itself is also recovery-safe: `spawnterm-tmux spawn …` uses
...
agent id (see `spawnterm-tmux name`), the same agent always maps to the same
...
spawnTerm agent, on (re)start, pulls its durable state from the broker:
