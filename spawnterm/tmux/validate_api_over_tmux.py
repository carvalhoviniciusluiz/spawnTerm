...
spawnTerm Tier 3 (#5). scope:external-tooling.
...
tmux, so they survive quit/disconnect/crash. The Tier 1 daemon (spawnterm/daemon)
...
--------------------------------------------------------
  ...
  3. Start a tmux -CC session running a shell, e.g. from an iTerm2 tab:
         spawnterm/tmux/spawnterm-tmux spawn --no-gate --role probe --task api -- $SHELL
     (or plainly:  tmux -CC new-session -A -s st-probe )
  4. Run this harness from another shell:
         python3 spawnterm/tmux/validate_api_over_tmux.py --session st-probe
...
TEMP_VAR = "user.spawnterm_tmux_probe"
...
async def _run(connection, session_substring: str) -> int:
    ...
    for key, val in results.items():
        ...
        if val.startswith("FAIL"):
            any_fail = True
    print("==========================================================")
    print("Paste this table into spawnterm/tmux/README.md (Findings). Do not")
