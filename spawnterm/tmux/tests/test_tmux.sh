#!/usr/bin/env bash
# Behavior tests for spawnterm-tmux (Tier 3, #5).
#
# spawnterm-tmux drives iTerm2 via osascript and, at run time, tmux — neither of
...
#
# Run from anywhere: bash spawnterm/tmux/tests/test_tmux.sh
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
TMUX_DIR="$(dirname "$HERE")"
TMUX_BIN="$TMUX_DIR/spawnterm-tmux"
FLAG="$(cd "$TMUX_DIR/../flags" && pwd)/spawnterm-flag"
EMIT="$(cd "$TMUX_DIR/../emit" && pwd)/spawnterm-emit"
...
echo "=== spawnterm-tmux behavior tests (dry-run / pure) ==="
...
CFG_DIR="$(mktemp -d)"
export SPAWNTERM_CONFIG="$CFG_DIR/config.toml"
: > "$SPAWNTERM_CONFIG"
...
echo
echo "--- 2. gate OFF (default) => delegate to spawnterm-spawn, NO tmux ---"
off_out="$(sh "$TMUX_BIN" spawn --role worker --task 'build #5' --dry-run -- claude --resume)"
assert_contains "announces delegation"          "delegating to spawnterm-spawn" "$off_out"
assert_contains "runs the spawnterm-spawn plan" "spawnterm-spawn: DRY RUN"       "$off_out"
...
echo
echo "--- 3. gate ON (config: spawnterm.tmux=true) => tmux -CC wrapping ---"
sh "$FLAG" enable spawnterm.tmux >/dev/null
...
assert_contains "gate reported ON"                  "gate         : ON (spawnterm.tmux)" "$on_out"
...
assert_absent   "no SPAWNTERM_PORT export (iso off)" "SPAWNTERM_PORT"                    "$on_out"
...
sh "$FLAG" disable spawnterm.tmux >/dev/null
...
# SPAWNTERM_FORCE=1 also bypasses.
force_out="$(SPAWNTERM_FORCE=1 sh "$TMUX_BIN" spawn --role worker --task t --dry-run -- claude)"
assert_contains "SPAWNTERM_FORCE=1 forces the tmux path" "tmux -CC new-session" "$force_out"
...
assert_exit "spawn dry-run happy path exits 0"   0 env SPAWNTERM_FORCE=1 sh "$TMUX_BIN" spawn --role x --dry-run -- true
...
if command -v osacompile >/dev/null 2>&1; then
	for label in "tmux-spawn" "attach"; do
		...
		if [ "$label" = "tmux-spawn" ]; then
			SPAWNTERM_FORCE=1 sh "$TMUX_BIN" spawn --role worker --task "o'brien #5" --dry-run -- claude --resume \
