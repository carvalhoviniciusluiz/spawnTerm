# spawnterm-emit — lifecycle palette & color/badge escape codes

Tier 0.2 (#8) extends `spawnterm-emit` with two subcommands that theme a
...
exact bytes emitted. Both the shell (`spawnterm-emit`) and Python
(`spawnterm_emit.py`) implementations emit byte-identical sequences.
...
`spawnterm-emit` — `spawnterm.status_board`, checked via the `spawnterm-flag`
helper (#11) through the shared `gate_open` code path (no second gate). When the
flag is OFF, absent, or `spawnterm-flag` is not on `PATH`, they emit nothing and
exit `0`. Bypass for local testing with `--no-gate` or `SPAWNTERM_FORCE=1`.
