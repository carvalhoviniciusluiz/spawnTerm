#!/usr/bin/env bash
# Behavior tests for spawnterm-review (Phase 2, #14).
#
# Covers:
#   1. resolve (PURE): reuses the #13 allocator so the derived branch/worktree
#      match `spawnterm-worktree plan`, and base auto-detects to main.
...
# Run: bash spawnterm/review/tests/test_review.sh
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
REVIEW_DIR="$(dirname "$HERE")"
RV="$REVIEW_DIR/spawnterm-review"
NOTIFY="$REVIEW_DIR/review_notify.py"
WT="$REVIEW_DIR/../spawn/spawnterm-worktree"
FLAG="$REVIEW_DIR/../flags/spawnterm-flag"
...
echo "=== spawnterm-review behavior tests ==="
echo "tool: $RV"
...
CFG="$TMP/config.toml"
export SPAWNTERM_CONFIG="$CFG"     # absent file => flag OFF
...
"$FLAG" enable spawnterm.review >/dev/null 2>&1
assert_contains "gate ON via config: flag listed on" "spawnterm.review               on" "$("$FLAG" list)"
...
if git -C "$REPO" log --oneline main | grep -q "spawnterm-review: merge $cbr into main"; then
...
export SPAWNTERM_BROKER_SOCK="$BSOCK"
...
export SPAWNTERM_BROKER_SOCK="$TMP/nonexistent.sock"
...
assert_contains "no-broker fallback reports the note file path" ".spawnterm-review/CHANGES-REQUESTED-" "$out_fb"
if ls "$nwt/.spawnterm-review/"CHANGES-REQUESTED-*.md >/dev/null 2>&1; then
	green "fallback wrote a note file into the agent worktree"
	if grep -q "add tests for the parser" "$nwt/.spawnterm-review/"CHANGES-REQUESTED-*.md; then
