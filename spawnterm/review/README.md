# spawnterm-review — per-agent diff/review surface (#14)
...
to look at the diff and decide *merge* or *send back*. `spawnterm-review` gives a
...
```
spawnterm-review resolve         <agent> [--role R]        # pure: branch/worktree/base
spawnterm-review show            <agent> [--role R]        # diff vs base (--stat + patch)
spawnterm-review approve         <agent> [--role R] [--cleanup]
spawnterm-review request-changes <agent> "<note>" [--role R]
spawnterm-review pane            <agent> [--role R]        # open show in an iTerm2 split
```

## How the target is resolved (reuses #13)

You address an agent the same way you spawned it — by **id (+ role)**.
`spawnterm-review` calls the #13 pure allocator (`spawnterm-worktree plan --id
<id> --role <role>`) to derive the **same** branch (`spawnterm/<slug>-<hash6>`)
...
(`spawnterm-review: merge <branch> into <base>`) that records the review event;
pass `--ff-only` to fast-forward instead. Any refusal merges **nothing** and
exits `1`; a failed merge is auto-aborted.

`--cleanup` runs #13's `spawnterm-worktree cleanup` after a successful merge
...
- **Broker (preferred, #34/#35).** If a broker socket is reachable
  (`--broker-sock`, else `$SPAWNTERM_BROKER_SOCK`, else the XDG default) and
...
- **Fallback (no broker).** If the broker is unreachable, or you pass
  ...
  `<worktree>/.spawnterm-review/CHANGES-REQUESTED-<timestamp>.md` (reviewer,
  branch, base, and the message) **and** printed. The agent — or you — sees it in
  the worktree.

`--from` sets the reviewer id (default `$SPAWNTERM_REVIEWER`, else `$USER`, else
...
```sh
spawnterm-review pane <agent> --role <role>
```

runs, in the current window, the AppleScript equivalent of *split the current
session vertically and run `spawnterm-review show <agent> --role <role>` in the
new pane* — via `osascript` (the same approach as #10's `spawnterm-spawn`; no
...
Everything gates on **`spawnterm.review`** (default **OFF**, like every spawnTerm
capability). The actions (`show` / `approve` / `request-changes` / `pane`)
self-gate via `spawnterm-flag`; `resolve` is pure and never gates.

```sh
spawnterm-flag enable spawnterm.review
```
...
testing with `--no-gate` or `SPAWNTERM_FORCE=1` — the same convention as
`spawnterm-emit` / `-spawn` / `-worktree`. The flag is registered in both flag
implementations (`spawnterm/flags/spawnterm-flag` and `spawnterm_flag.py`) and
documented in `spawnterm/docs/feature-flags.md`.

## Interop

- **#13 worktrees** (`spawnterm/spawn/spawnterm-worktree`, `WORKTREE.md`): review
...
- **Broker #34/#35** (`spawnterm/broker`): `request-changes` routes notes through
  the broker `send` op (durable mailbox with ack/replay) when it is running.

## Testing

```sh
bash spawnterm/review/tests/test_review.sh
```
