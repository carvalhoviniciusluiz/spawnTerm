#!/usr/bin/env python3
"""spawnterm-broker — Tier 2.1 durable-state broker (#34).
...
``spawnterm/docs/design.md`` — "What iTerm2 CANNOT do → external broker").
...
Feature-flag gate: like every spawnTerm capability, the broker is off by
default. The **server** starts only when ``spawnterm.broker`` is ON (checked via
the #11 flag helper). If the flag is OFF/absent it prints a clear message and
exits 0. Bypass for local testing with ``--no-gate`` or ``SPAWNTERM_FORCE=1`` —
...
PROG = "spawnterm-broker"
FLAG_KEY = "spawnterm.broker"
...
def flag_enabled() -> bool:
    """Return True iff ``spawnterm.broker`` is ON.
...
def _build_logger(verbose: bool) -> logging.Logger:
    logger = logging.getLogger("spawnterm.broker")
    logger.setLevel(logging.DEBUG if verbose else logging.INFO)
    handler = logging.StreamHandler(sys.stderr)
    handler.setFormatter(
        logging.Formatter("%(asctime)s %(levelname)s spawnterm.broker: %(message)s")
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=PROG,
        description="spawnTerm Tier 2 broker: durable sqlite state + unix-socket "
    ...
    serve.add_argument(
        "--no-gate",
        action="store_true",
        help="bypass the spawnterm.broker feature-flag gate (local testing).",
    )
