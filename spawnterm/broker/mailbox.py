#!/usr/bin/env python3
"""Durable per-agent mailbox for the spawnTerm broker (#35, Tier 2.2).
...
**the** spawnTerm differentiator: tmux ``send-keys`` fanout is ~70-80% reliable
...
recipient acks it (see ``spawnterm/docs/design.md`` — "What iTerm2 CANNOT do").
...
# this module. No per-op flag gate — the server already gates on spawnterm.broker.
# --------------------------------------------------------------------------- #
