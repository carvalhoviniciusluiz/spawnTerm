#!/usr/bin/env python3
"""Daemon↔broker bridge for spawnTerm (Tier 2.4, #37) — the glue that wires the
...
Two feature flags interact (see the daemon README for the full table):
  ...
  * ``spawnterm.messaging`` gates routing/mailbox — the *whole* relay. OFF ⇒ the
    daemon still parses/logs envelopes (upstream) but neither routes nor enqueues.
  * ``spawnterm.broker`` gates the *durable* path. The durable mailbox+registry
...
BROKER_FLAG = "spawnterm.broker"

# Where the in-repo broker package (client.py etc.) lives, relative to us:
# spawnterm/broker. Added to sys.path lazily by connect_broker so the pure path
# never needs it.
...
def marker_for(msg_id: Any) -> str:
    ...
    return f"[spawnterm#{msg_id}]"


def broker_enabled() -> bool:
    """Return True iff the ``spawnterm.broker`` flag is ON.
    ...
    try:
        ...
        return spawnterm_flag.is_enabled(BROKER_FLAG)
...
def format_durable_delivery(
    ...
    The line carries the unique ``[spawnterm#<id>]`` marker (so ack-by-observation
...
def was_observed(screen_text: Optional[str], marker: str) -> bool:
    ...
    ``marker`` (``[spawnterm#<id>]``) appears somewhere in that text — i.e. the
...
def connect_broker(logger: Any = None) -> Any:
    ...
    Adds the ``spawnterm/broker`` dir to ``sys.path`` (lazily — the pure path
    ...
    log = logger or logging.getLogger("spawnterm.daemon")
    try:
