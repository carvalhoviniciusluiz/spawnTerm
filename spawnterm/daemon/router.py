#!/usr/bin/env python3
"""In-memory best-effort message router for the spawnTerm daemon (Tier 1.3, #28).
...
Undeliverable outcomes (never raises; a bad envelope is logged and dropped):
  * ``messaging disabled`` — the ``spawnterm.messaging`` gate is OFF.
...
text carries a ``[spawnterm]`` marker and the sender id so the receiving agent
can see it is a spawnterm relay rather than local input.
"""
...
# Gate: like every spawnTerm capability this relay is off by default. We reuse
# the #11 flag helper (the same mechanism spawnterm_daemon.flag_enabled uses),
...
MESSAGING_FLAG = "spawnterm.messaging"
...
# spawnterm message apart from ordinary local input.
DELIVERY_PREFIX = "[spawnterm]"
...
def messaging_enabled() -> bool:
    """Return True iff the ``spawnterm.messaging`` flag is ON.

    Mirrors ``spawnterm_daemon.flag_enabled``: import the #11 helper and ask it.
    ...
    try:
        ...
        return spawnterm_flag.is_enabled(MESSAGING_FLAG)
...
def format_delivery(sender: str | None, body_text: str) -> str:
    ...
    ``[spawnterm]`` marker and the sender so the receiver knows it is a relay."""
...
def route_if_enabled(envelope, registry, *, enabled: bool) -> RoutingDecision:
    """Gate then route. When ``enabled`` is False (the ``spawnterm.messaging``
