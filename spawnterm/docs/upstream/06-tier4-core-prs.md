...
and ack live in spawnTerm's external Tier 2 broker (sqlite + unix socket), by design. We ask iTerm2
...
spawnTerm's Tier 2 bridge delivers a routed message into a target session with `async_send_text`,
then confirms delivery by **observing the target's screen** for a unique marker before `ack`-ing the
durable mailbox (`spawnterm/daemon/bridge.py`, ack-by-observation). That screen-scrape is a
...
### Evidence from the working tooling
Both the Tier 1 daemon (`spawnterm/daemon/registry.py`, ephemeral) and the Tier 2 broker
(`spawnterm/broker/store.py`, persistent) maintain their **own** agent registry because the API
...
### Evidence from the working tooling
spawnTerm identity lives in dot-free user-vars `user.agent_status` / `agent_role` / `agent_task` /
`agent_id` (set by `spawnterm-emit`; consumed by the badge, the daemon dashboard, and the review /
...
sidecar would make per-session identity durable for any script, not just spawnTerm.
