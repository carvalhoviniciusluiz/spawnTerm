# spawnTerm — cooperation strategy (web-research informed, 2026)

Companion to `native-vs-spawnterm.md`. This folds in external research on the current landscape and
...
3. **spawnTerm** (this fork's external tooling + core additions).

So the founding premise ("terminals have no agent orchestration") is now **false twice over** — both
iTerm2 and Claude Code ship orchestration. spawnTerm must stop duplicating them and become the
**durable, runtime-isolation layer underneath both**.

## What the research validates as genuinely ours (the moat)
Both native systems share the **same two documented gaps** — and both are exactly what spawnTerm owns:
...
- **Durability of coordination state.** Claude Code agent teams (per community + Anthropic docs) have
  ...
  resumes with a fresh context). → **spawnTerm Tier 2 broker** (durable sqlite mailbox + ack + replay +
...
- **Runtime isolation, not just file isolation.** Claude Code's native `isolation: worktree` for
  ...
  bugs. → **spawnTerm #13** ($SPAWNTERM_PORT/$NS) fills this, and neither native system does.
...
- **Align `spawnterm-emit` to cc-status / OSC 21337** so spawnTerm-spawned agents light up the **native
...
- **Bridge the broker under Claude Code agent teams:** a hook (`TeammateIdle`/`TaskCompleted` or a
  wrapper) mirrors the team's task/coordination state into spawnTerm's **durable broker**, so it
...
- **Runtime isolation for the panes both native systems spawn:** wrap Claude Code/iTerm2 split-pane
  teammates with spawnTerm's worktree+$PORT so parallel teammates don't collide on ports/DBs.
...
- **In-memory router (#28) as a standalone path: RETIRE** — Claude Code teams already message; keep
  spawnTerm messaging on the **durable broker** only (in-memory relay adds nothing durable).
...
1. Align `spawnterm-emit` → cc-status/OSC 21337 (cooperate).
...
**Positioning in one line:** spawnTerm = the **durable coordination + runtime-isolation substrate** that
