...
spawnterm.agent_inbox (default OFF), honors --no-gate / SPAWNTERM_FORCE, and no
...
class TestGate(unittest.TestCase):
    def setUp(self):
        ...
        self._prev_cfg = os.environ.get("SPAWNTERM_CONFIG")
        self._prev_force = os.environ.get("SPAWNTERM_FORCE")
        os.environ["SPAWNTERM_CONFIG"] = os.path.join(self._tmp.name, "config.toml")
    ...
    def tearDown(self):
        if self._prev_cfg is None:
            ...
            os.environ["SPAWNTERM_CONFIG"] = self._prev_cfg
        if self._prev_force is not None:
            os.environ["SPAWNTERM_FORCE"] = self._prev_force
        self._tmp.cleanup()
    ...
    def test_flag_on_opens_gate(self):
        ...
        spawnterm_flag = importlib.import_module("spawnterm_flag")
        spawnterm_flag._set("agent_inbox", True)  # writes SPAWNTERM_CONFIG
