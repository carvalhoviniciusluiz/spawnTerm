...
* ``spawnterm.inbox.requests``  — one message per submitted request. The broker
  message id *is* the request id (monotonic, unique).
* ``spawnterm.inbox.decisions`` — one message per decision, body carries the
...
REQUESTS_ADDR = "spawnterm.inbox.requests"
DECISIONS_ADDR = "spawnterm.inbox.decisions"
INBOX_SENDER = "spawnterm.inbox"
