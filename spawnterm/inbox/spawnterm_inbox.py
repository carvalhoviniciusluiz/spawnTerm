...
Gating: every subcommand honors ``spawnterm.agent_inbox`` (default OFF). When the
flag is off the inbox is a **no-op** — commands print a one-line notice and exit
0 without touching the broker. Bypass with ``--no-gate`` or ``SPAWNTERM_FORCE=1``.
...
PROG = "spawnterm-inbox"

# Sibling broker package: spawnterm/broker/client.py.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog=PROG, description="spawnTerm human-attention inbox.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        print(f"{PROG}: the inbox queue is durable broker state; start the broker "
              f"(spawnterm_broker.py serve) or pass --in-memory for a non-durable run.",
              file=sys.stderr)
        return 1
