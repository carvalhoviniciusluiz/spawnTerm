#!/usr/bin/env bash
# Behavior tests for spawnterm-worktree (Phase 2, #13).
#
# Covers:
...
# Run: bash spawnterm/spawn/tests/test_worktree.sh
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
SPAWN_DIR="$(dirname "$HERE")"
WT="$SPAWN_DIR/spawnterm-worktree"
...
echo "=== spawnterm-worktree behavior tests ==="
echo "helper: $WT"

# A throwaway git repo so we exercise real `git worktree` without touching the
# host repo. All worktrees land under $ROOT/wt (SPAWNTERM_WORKTREE_ROOT).
...
assert_contains "branch is under spawnterm/ + sanitized slug" "branch=spawnterm/worker-13-" "$p1"
assert_contains "namespace is DB-safe (role + hash, underscore)" "namespace=worker_" "$p1"
assert_contains "worktree path honors SPAWNTERM_WORKTREE_ROOT" "worktree=$TMP/wt/worker-13-" "$p1"
port1="$(val port "$p1")"
...
assert_contains "uppercase/spaces/punct collapse to a safe branch slug" "branch=spawnterm/tech-lead-feat-13-ports-" "$messy"
...
assert_contains "env exports SPAWNTERM_PORT"     "export SPAWNTERM_PORT="     "$env_out"
assert_contains "env exports SPAWNTERM_NS"       "export SPAWNTERM_NS="       "$env_out"
assert_contains "env exports SPAWNTERM_WORKTREE" "export SPAWNTERM_WORKTREE=" "$env_out"
assert_contains "env exports SPAWNTERM_BRANCH"   "export SPAWNTERM_BRANCH="   "$env_out"

echo
echo "--- 2. gate: OFF => create no-ops (exit 0), ON via config => runs ---"
# Point the flag helper at a config we control. Absent flag => OFF.
CFG="$TMP/config.toml"
export SPAWNTERM_CONFIG="$CFG"
...
"$SPAWN_DIR/../flags/spawnterm-flag" enable spawnterm.worktree_isolation >/dev/null 2>&1
on_out="$(sh "$WT" create --repo "$REPO" --id gatecheck --role worker 2>/dev/null)"
assert_contains "gate ON (config): create emits the allocation" "branch=spawnterm/worker-gatecheck-" "$on_out"
...
assert_contains "dry-run still prints the allocation"       "branch=spawnterm/worker-dryagent-" "$dry"
