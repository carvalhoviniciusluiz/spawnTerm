#!/usr/bin/env python3
"""spawnterm-mcp — MCP orchestration surface for spawnTerm (#18).

Exposes spawnTerm's orchestration (spawn / assign / handoff / send_message /
...
or state (see ``spawnterm/docs/design.md``).
...
This file is the **thin I/O shell** only:
  ...
  * the feature-flag gate (``spawnterm.mcp``, default OFF — refuse to start with
    a message + exit 0, bypassable with ``--no-gate`` / ``SPAWNTERM_FORCE=1``,
...
# --------------------------------------------------------------------------- #
# Feature-flag gate (identical convention to spawnterm-broker / spawnterm-daemon)
# --------------------------------------------------------------------------- #


def flag_enabled() -> bool:
    """Return True iff ``spawnterm.mcp`` is ON.

    Prefers the #11 Python helper (``spawnterm_flag.is_enabled``); falls back to
    shelling out to the ``spawnterm-flag`` script. Fail-safe: if neither is
...
def _build_logger(verbose: bool) -> logging.Logger:
    logger = logging.getLogger("spawnterm.mcp")
    logger.setLevel(logging.DEBUG if verbose else logging.INFO)
    handler = logging.StreamHandler(sys.stderr)
    handler.setFormatter(
        logging.Formatter("%(asctime)s %(levelname)s spawnterm.mcp: %(message)s")
    )
...
def _make_spawn_launcher(logger: logging.Logger):
    ...
    imports ``iterm2``. The daemon spawn CLI (``spawnterm_daemon.py spawn``)
    ...
    """
    daemon_cli = _DAEMON_DIR / "spawnterm_daemon.py"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog=PROG,
        description="spawnTerm MCP surface: orchestration tools (spawn/assign/"
    ...
    parser.add_argument(
        "--no-gate",
        action="store_true",
        help="bypass the spawnterm.mcp feature-flag gate (local testing).",
    )
