#!/usr/bin/env python3
"""Tests for the cost CLI/I/O layer + feature-flag gate (spawnTerm #16).

Proves: the gate is default-OFF (no-op, exit 0, nothing on stdout), the
``--no-gate``/``SPAWNTERM_FORCE`` bypasses work, the source is configurable and
files are discovered/parsed end to end, and the rendered table + JSON reflect
the fixture numbers. Uses an isolated ``SPAWNTERM_CONFIG`` so it never reads a
real ~/.config.
"""
...
class GateTestBase(unittest.TestCase):
    def setUp(self):
        ...
        self._old_cfg = os.environ.get("SPAWNTERM_CONFIG")
        os.environ["SPAWNTERM_CONFIG"] = self._cfg
        self._old_force = os.environ.pop("SPAWNTERM_FORCE", None)
        self._old_src = os.environ.pop("SPAWNTERM_COST_SOURCE", None)
        self._old_prices = os.environ.pop("SPAWNTERM_COST_PRICES", None)

    def tearDown(self):
        for key, val in (
            ("SPAWNTERM_CONFIG", self._old_cfg),
            ("SPAWNTERM_FORCE", self._old_force),
            ("SPAWNTERM_COST_SOURCE", self._old_src),
            ("SPAWNTERM_COST_PRICES", self._old_prices),
        ):
            if val is None:
