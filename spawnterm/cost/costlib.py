#!/usr/bin/env python3
"""spawnTerm cost/token dashboard — pure core (spawnTerm #16).

This module is the *pure*, testable heart of the cost dashboard. It has **no**
I/O of its own (the CLI in ``spawnterm-cost`` reads files and hands lines here)
...
def agent_key(entry: UsageEntry, group_by: str = DEFAULT_GROUP_BY) -> str:
    ...
    does not carry the spawnTerm agent id, so we group usage by a proxy:

    * ``cwd`` (default): the basename of the working directory. spawnTerm gives
