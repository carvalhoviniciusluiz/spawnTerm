...
//   - When ON, the upstream feature is available: the independent menu bar
//     item (gated additionally by the ShowMenuBarItem Advanced setting) and
//     the orange busy-agent count badge.
...
class AIMenuBarStatusController: NSObject {
    ...
    private func refreshOnMain() {
        ...
        // Legacy stock behavior: excluded from Dock + status bar icon enabled.
        // Preserved regardless of the flag so turning the flag OFF is a no-op
        // relative to stock iTerm2.
        ...
        if shouldShow {
            ...
            // The busy-count badge is part of the gated feature; when the flag
            // is OFF the (legacy) item shows the plain template image only.
            updateImage(showBadge: flagOn)
        } else {
            removeStatusItem()
        }
    }
    ...
    private func busyCount() -> Int {
        // TypingStatusModel is main-actor isolated; refreshOnMain always runs
        // on the main queue, so assume isolation here.
    ...
    private func updateImage(showBadge: Bool) {
        guard let button = statusItem?.button else { return }
        let count = showBadge ? busyCount() : 0
        if count == 0 {
