# it2agent-janitor — verification/merge gate (#15)
...
`it2agent-janitor` is a self-contained `sh` tool (matching the #13/#14 suite).
It shells out only to `git`, the sibling `it2agent-worktree` (#13) and
`it2agent-flag` (#11), and the **project-provided** gate commands. It never
touches iTerm2 source.

```
it2agent-janitor <command> [<agent-id>] [options]
```
...
| `check`   | side-effecting | **yes** (`agent.janitor`) | Run the gate in the worktree; block on failure. |
...
2. `$IT2AGENT_GATE_CONFIG` — same, from the environment.
3. `<repo>/.it2agent/gate.toml` — repo-local TOML `[gate]` table.
4. `<repo>/it2agent-gate.sh` — a repo-local executable; the **whole** gate is
...
```
it2agent-janitor config --repo /path/to/repo
```
```
repo=/path/to/repo
gate-source=config:/path/to/repo/.it2agent/gate.toml
...
When the `agent.janitor` flag is **OFF**, `check` is a no-op: it prints that
...
`<worktree>/.it2agent-janitor/summary.txt`:

```
repo=/path/to/repo
worktree=/path/to/wt/worker-ok15-ab12cd
branch=it2agent/worker-ok15-ab12cd
gate-source=config:/path/to/gate.toml
check=lint status=pass
check=test status=fail exit=1 log=/path/to/wt/.../.it2agent-janitor/test.log
overall=blocked
```
...
`<worktree>/.it2agent-janitor/<check>.log`. For **failing** checks the janitor
...
```
agent=backend
map=/repo/.it2agent/ownership.toml
branch=it2agent/worker-backend-ab12cd
base=main
owned-globs=src/api/**
changed-files=3
eligible=yes
...
`check` self-gates on **`agent.janitor`** (seeded in #11, default OFF),
consulted via `it2agent-flag`:

```
it2agent-flag enable agent.janitor    # turn the janitor on
```
...
### #13 — worktrees (`it2agent-worktree`)

`check`/`owns`/`resolve` reuse the #13 allocator (`it2agent-worktree plan`) to
...
### #14 — review surface (`it2agent-review`)

`it2agent-review approve` (or any auto-merge flow) consults the janitor **before
merging**:

```sh
# gate must pass ...
it2agent-janitor check "$agent" --role "$role" --repo "$repo" || exit 1
# ... and (for unattended auto-merge) the diff must be owned:
it2agent-janitor owns "$agent" --role "$role" --repo "$repo" || exit 1
it2agent-review approve "$agent" --role "$role" --repo "$repo"
```
...
```
bash it2agent/janitor/tests/test_janitor.sh
```
