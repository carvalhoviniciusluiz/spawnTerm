#!/usr/bin/env bash
# Behavior tests for it2agent-janitor (Phase 3, #15).
#
# Covers:
...
#      a repo-local it2agent-gate.sh becomes the whole gate; empty gate.
#   2. RESOLVE (pure): reuses the #13 allocator so branch/worktree match
#      `it2agent-worktree plan`; base auto-detects to main.
...
# Run: bash it2agent/janitor/tests/test_janitor.sh
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
JAN_DIR="$(dirname "$HERE")"
JN="$JAN_DIR/it2agent-janitor"
WT="$JAN_DIR/../spawn/it2agent-worktree"
FLAG="$JAN_DIR/../flags/it2agent-flag"
...
echo "=== it2agent-janitor behavior tests ==="
echo "tool: $JN"
...
# 1d. a repo-local it2agent-gate.sh becomes the whole gate.
SREPO="$TMP/script"; mkdir -p "$SREPO"; git -C "$SREPO" init -q
printf '#!/bin/sh\nexit 0\n' > "$SREPO/it2agent-gate.sh"; chmod +x "$SREPO/it2agent-gate.sh"
script_out="$(sh "$JN" config --repo "$SREPO")"
assert_contains "repo-local script becomes the gate source" "gate-source=script:$SREPO/it2agent-gate.sh" "$script_out"
assert_contains "repo-local script is the single 'gate' check" "check=gate cmd=$SREPO/it2agent-gate.sh" "$script_out"
...
export IT2AGENT_CONFIG="$TMP/flags.toml"   # absent file => flag OFF
...
"$FLAG" enable agent.janitor >/dev/null 2>&1
assert_contains "gate ON via config: flag listed on" "agent.janitor                  on" "$("$FLAG" list)"
...
if ls "$dwt/.it2agent-janitor/summary.txt" >/dev/null 2>&1; then
...
if grep -q "overall=blocked" "$badwt/.it2agent-janitor/summary.txt" 2>/dev/null; then
	green "summary.txt persisted with overall=blocked"
else
	red "summary.txt missing or wrong"
fi
if grep -q "BOOM-the-tests-failed" "$badwt/.it2agent-janitor/test.log" 2>/dev/null; then
