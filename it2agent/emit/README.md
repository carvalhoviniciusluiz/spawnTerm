...
> [`it2agent/spawn`](../spawn/README.md) (#10), which calls these commands.

Two byte-for-byte-identical implementations are provided so agents can use
whichever is convenient:

- `it2agent-emit` — pure POSIX shell
- `it2agent_emit.py` — Python 3 (no external deps)

## Usage

```
it2agent-emit [--no-gate] <command> [args]
```
...
`it2agent: agent needs attention`.
...
Like every it2agent capability, the emitter gates on its feature flag —
`agent.status_board` — checked via the `it2agent-flag` helper (#11):

- Emits only when `it2agent-flag agent.status_board` reports ON (exit 0).
- If the flag is OFF, or `it2agent-flag` is not on `PATH` (fail-safe:
...
The coupling is loose: it shells out to `it2agent-flag` (never imports #11's
internals), so the two land independently.

## Tests

```
bash it2agent/emit/tests/test_emit.sh
```
