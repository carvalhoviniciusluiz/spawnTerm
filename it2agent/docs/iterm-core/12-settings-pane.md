# Implementation plan — it2agent capability toggles in Settings → General → AI (issue #12, fork-direct)
...
## Problem
it2agent capabilities are per-user feature flags (default OFF) stored in
`~/.config/it2agent/config.toml` under a `[features]` table and read/written by
`it2agent/flags/` (`it2agent-flag` shell + `it2agent_flag.py`). Today users toggle them by CLI
...
iTerm2 prefs normally live in `NSUserDefaults`/`iTermPreferences`; it2agent flags live in an
...
A new "it2agent" box in the AI **Features** pane with one `kPreferenceInfoTypeCheckbox` per
capability (`status_board`, `worktree_isolation`, `messaging`, `inbox`, `cost_dashboard`,
`janitor`, `mcp`, `daemon`, `broker`, `review`, `tmux` — the current `KNOWN_FLAGS`). Each checkbox's:
- `syntheticGetter` → returns the flag state (shell out to `it2agent-flag <cap>`, exit 0 = on; or
  read the TOML directly). Batch via `it2agent-flag list` to avoid one shell per checkbox refresh.
- `syntheticSetter` → `it2agent-flag enable/disable <cap>`.
...
- **Cost:** shelling to `it2agent-flag` per refresh — mitigate with a single `list` call.
...
pane bound to `it2agent-flag`, matching the surrounding fixed-frame layout, build with
