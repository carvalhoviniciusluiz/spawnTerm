# it2agent — iTerm2 core change plans (fork-direct)

These are the **`scope:iterm2-core`** items: they change **iTerm2 source**. Per the **Fork policy**
(the pinned "Fork policy" comment on Epic #1), it2agent is a **personal fork** — changes to iTerm2
...
1. It edits files **outside `it2agent/`** (the iTerm2 source tree). Follow `CLAUDE.md` strictly
...
iTerm2 is the **substrate/transport**; it2agent's external broker (Tier 2) owns the durable
...
- [`12-settings-pane.md`](12-settings-pane.md) — issue #12: a it2agent capability section in
  **Settings → General → AI** (checkboxes bound to the `agent.<key>` flags).
