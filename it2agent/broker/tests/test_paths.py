...
_VARS = ("IT2AGENT_BROKER_DB", "IT2AGENT_BROKER_SOCK", "XDG_STATE_HOME", "XDG_RUNTIME_DIR")
...
class TestPaths(unittest.TestCase):
    ...
    def test_db_explicit_override_wins(self):
        os.environ["IT2AGENT_BROKER_DB"] = "/tmp/x/custom.db"
    ...
    def test_db_uses_xdg_state_home(self):
        os.environ["XDG_STATE_HOME"] = "/tmp/state"
        self.assertEqual(paths.broker_db_path(), Path("/tmp/state/it2agent/broker.db"))

    def test_db_fallback_to_local_state(self):
        self.assertEqual(
            paths.broker_db_path(), Path.home() / ".local/state/it2agent/broker.db"
        )

    def test_sock_explicit_override_wins(self):
        os.environ["IT2AGENT_BROKER_SOCK"] = "/tmp/x/custom.sock"
    ...
    def test_sock_uses_xdg_runtime_dir(self):
        os.environ["XDG_RUNTIME_DIR"] = "/tmp/run"
        self.assertEqual(paths.broker_sock_path(), Path("/tmp/run/it2agent/broker.sock"))

    def test_sock_fallback_to_local_state(self):
        self.assertEqual(
            paths.broker_sock_path(), Path.home() / ".local/state/it2agent/broker.sock"
        )
