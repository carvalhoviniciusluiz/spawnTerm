#!/usr/bin/env python3
"""Per-user path resolution for the it2agent broker (#34).
...
Precedence for the database:
  ``$IT2AGENT_BROKER_DB`` (full path, mainly for tests) >
  ``$XDG_STATE_HOME/it2agent/broker.db`` >
  ``~/.local/state/it2agent/broker.db``

Precedence for the socket:
  ``$IT2AGENT_BROKER_SOCK`` (full path, mainly for tests) >
  ``$XDG_RUNTIME_DIR/it2agent/broker.sock`` >
  ``~/.local/state/it2agent/broker.sock``
...
def broker_db_path() -> Path:
    ...
    override = _env("IT2AGENT_BROKER_DB")
    if override:
...
def broker_sock_path() -> Path:
    ...
    override = _env("IT2AGENT_BROKER_SOCK")
    if override:
        return Path(override).expanduser()
