#!/usr/bin/env python3
"""it2agent cost dashboard — CLI + I/O layer (it2agent #16).
...
* discovers Claude Code transcript JSONL under a configurable source
  ...
  ``$IT2AGENT_COST_SOURCE``);
* reads the price table (default table in :mod:`costlib`, override via
  ``--prices`` / ``$IT2AGENT_COST_PRICES`` pointing at a JSON file);
...
* renders a table (or ``--json``), flags idle-burn, and — on a soft-cap breach
  or idle-burn — prints a warning and optionally fires ``it2agent-emit
  attention`` (``--notify``).

Gating: no-op (exit 0) unless ``agent.cost_dashboard`` is ON, matching the
``it2agent-emit`` convention (``--no-gate`` / ``IT2AGENT_FORCE=1`` bypass).

The executable entry point is the sibling ``it2agent-cost`` script.
"""
...
def default_source() -> str:
    """The log source: ``$IT2AGENT_COST_SOURCE`` or the Claude default dir."""
    return os.environ.get("IT2AGENT_COST_SOURCE") or DEFAULT_SOURCE
...
def load_prices(prices_path: str | None):
    ...
    Precedence: explicit ``--prices`` > ``$IT2AGENT_COST_PRICES`` > defaults.
    A missing or malformed file falls back to the defaults (with a stderr note).
    """
    path = prices_path or os.environ.get("IT2AGENT_COST_PRICES")
    if not path:
...
def notify(messages: list[str]) -> None:
    """Fire ``it2agent-emit attention`` per message if the emitter is on PATH.

    Best-effort: the guaranteed channel is the stderr warning printed by the
    caller. The emitter honors its own ``agent.status_board`` gate.
    """
    emitter = shutil.which("it2agent-emit")
    if emitter is None:
        return
    for msg in messages:
        try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=PROG,
        description=(
            "Per-agent + total token/cost dashboard for a it2agent agent fleet, "
    ...
    parser.add_argument(
        "--source",
        default=None,
        help=f"log source dir or file (default: $IT2AGENT_COST_SOURCE or {DEFAULT_SOURCE}).",
    )
    parser.add_argument(
        "--group-by",
    ...
    parser.add_argument(
        "--prices",
        default=None,
        help="path to a JSON price-table override (default: $IT2AGENT_COST_PRICES or built-in).",
    )
    parser.add_argument(
        "--idle-gap",
        type=float,
    ...
    parser.add_argument(
        "--notify",
        action="store_true",
        help="on a breach/idle-burn, also fire it2agent-emit attention (best-effort).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.notify and warnings:
        notify([f"it2agent-cost: {w}" for w in warnings])
