# it2agent/cost — per-agent token/cost dashboard (#16)

Multi-agent fleets burn ~7× the tokens of a single session, and idle agents keep
spending (background compaction, polling, a human who walked away). it2agent's
...
Configure the source with `--source <dir-or-file>` or `$IT2AGENT_COST_SOURCE`
...
```
$ it2agent-cost --prices my-prices.json      # or $IT2AGENT_COST_PRICES
```
...
A raw Claude transcript does **not** carry the it2agent agent id, so usage is
associated with an agent by a configurable proxy (`--group-by`):

| key       | agent label                                   | rationale |
|-----------|-----------------------------------------------|-----------|
| `cwd` (default) | basename of the working directory       | it2agent gives each agent an isolated worktree (#13), so the leaf dir is a good agent proxy |
...
`--notify` a best-effort `it2agent-emit attention` notification fires (OSC 9).
**Soft only — never kills anything.** With no cap flags, nothing is evaluated.

## Surfacing

* **CLI** — `it2agent-cost` prints the per-agent + total table (or `--json`),
...
* **Emitter** — soft-cap/idle-burn notifications route through `it2agent-emit`.

## Feature flag

Everything gates on **`agent.cost_dashboard`** (seeded in #11, default OFF),
consumed via the shared `it2agent-flag` helper. When OFF the CLI is a **no-op**
...
| `it2agent-cost`| thin executable entry point → `cost_cli.main`. |
| `cost_board.py` | status-bar surface: pure formatter + lazy-`iterm2` component (#29 pattern). |
| `tests/`        | unit tests + fixture JSONL (`agent_alpha`, `agent_beta`, `malformed`) and a `prices_override.json`. Run: `bash it2agent/cost/tests/run_tests.sh`. |

## Testing

```
bash it2agent/cost/tests/run_tests.sh
```
