#!/usr/bin/env python3
"""it2agent cost dashboard — status-bar surface (it2agent #16).

Mirrors the #29 daemon status-bar pattern (``it2agent/daemon/dashboard.py``):
...
(``it2agent-cost``) is the full-table surface.
...
Gate: registered only when ``agent.cost_dashboard`` is ON (default OFF),
...
COMPONENT_IDENTIFIER = "com.agent.cost-dashboard"
COMPONENT_SHORT_DESCRIPTION = "it2agent Cost"
...
async def maybe_register_cost_board(connection, logger=None, source=None, group_by=None) -> bool:
    """Register the cost board **iff** ``agent.cost_dashboard`` is ON."""
