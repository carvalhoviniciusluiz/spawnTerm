...
defined once for the emitter in ``it2agent/emit/docs/colors.md`` (#8) and reused
...
Gate: the component is registered only when the ``agent.status_board``
feature flag is ON (default OFF). This reuses the shared ``it2agent-flag``
...
# it2agent/emit/docs/colors.md (#8): busy blue, blocked orange, done bluish
...
# ---------------------------------------------------------------------------
# Feature-flag gate (reuses the shared it2agent-flag helper, #11 — same
...
def status_board_enabled() -> bool:
    """Return True iff the ``agent.status_board`` flag is ON.

    Reuses the #11 ``it2agent_flag.is_enabled`` helper. Fail-safe: if the
...
def dashboard_gate_open() -> bool:
    ...
    ON when the ``agent.status_board`` flag is set, or bypassed locally with
    ``IT2AGENT_FORCE=1`` (matching the daemon's gate bypass).
    """
...
COMPONENT_IDENTIFIER = "com.agent.agent-dashboard"
COMPONENT_SHORT_DESCRIPTION = "it2agent Agent"
COMPONENT_DETAILED_DESCRIPTION = (
    "Shows this session's agent role and lifecycle status "
    "(and task if it fits), colored with the it2agent lifecycle palette."
)
...
async def maybe_register_dashboard(connection, logger=None) -> bool:
    """Register the dashboard **iff** the ``agent.status_board`` gate is open.
