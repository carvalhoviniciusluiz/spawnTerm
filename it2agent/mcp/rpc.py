#!/usr/bin/env python3
"""Pure JSON-RPC 2.0 / MCP request dispatch for the it2agent MCP surface (#18).
...
sockets. The stdio read/write loop lives in ``it2agent_mcp``; the tool handlers
...
SERVER_INFO = {"name": "it2agent", "version": "0.1.0"}
...
INSTRUCTIONS = (
    "it2agent orchestration tools. Every capability is a feature flag, default "
...
GUIDE_RESOURCE = {
    "uri": tools.GUIDE_URI,
    "name": "it2agent agent capability guide",
    "description": "AGENT_GUIDE.md — the single source of truth for every "
    "it2agent capability, its feature flag, command/MCP tool, and example.",
