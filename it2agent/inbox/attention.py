...
* :class:`AttentionEmitter` — the injectable side-effect. The default
  :class:`EmitAttentionEmitter` shells out to ``it2agent-emit attention <msg>``
...
reaches the pane through the ``it2agent-emit`` subprocess, exactly as the design
...
``it2agent-emit`` in the current process, which lands on the pane that hosts it
...
def build_message(request: Any) -> str:
    ...
    return (
        f"it2agent inbox: approval needed - {detail} "
...
class EmitAttentionEmitter(AttentionEmitter):
    """Default emitter: shell out to ``it2agent-emit attention <message>``.

    The subprocess writes the iTerm2 escape codes to *its* stdout, i.e. the pane
    that hosts it. ``it2agent-emit`` self-gates on ``agent.status_board`` and
