...
Config path precedence (mirrors the flags helper, #11):
  ``$IT2AGENT_INBOX_CONFIG`` (full path override, mainly for tests) >
  ``$XDG_CONFIG_HOME/it2agent/inbox.toml`` >
  ``~/.config/it2agent/inbox.toml``
...
def config_path() -> Path:
    ...
    override = _env("IT2AGENT_INBOX_CONFIG")
    if override:
        return Path(override).expanduser()
    base = _env("XDG_CONFIG_HOME")
    root = Path(base).expanduser() if base else Path.home() / ".config"
    return root / "it2agent" / "inbox.toml"
