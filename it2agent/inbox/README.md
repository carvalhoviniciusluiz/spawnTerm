...
## Feature flag: `agent.inbox`

Like every it2agent capability, an individually toggleable, per-user flag that
...
```sh
it2agent-flag enable agent.inbox     # turn it on
it2agent-flag agent.inbox            # query (1/exit0 = on)
```

The gate reuses the #11 `it2agent_flag.is_enabled` helper (falling back to the
`it2agent-flag` binary on PATH, then to OFF). Bypass locally with `--no-gate` or
`IT2AGENT_FORCE=1` — the same fail-safe convention as the emitter and daemon.
...
```
$IT2AGENT_INBOX_CONFIG  >  $XDG_CONFIG_HOME/it2agent/inbox.toml  >  ~/.config/it2agent/inbox.toml
```
...
to `needs_human`. `it2agent-inbox config-path` prints the resolved path.

## Intake path (how requests arrive)

An agent submits an `InboxRequest`. Two supported paths, same code:

* **CLI** — `it2agent-inbox submit --action … --scope … [--cost N] [--reversible]
...
* `EmitAttentionEmitter` (default) shells out to `it2agent-emit attention <msg>`,
  which writes `RequestAttention=yes` + an `OSC 9` notification. Because
  `it2agent-emit` self-gates on `agent.status_board`, it is safe to call
...
`it2agent-emit` subprocess, per the design note.

## Approve / edit / reject workflow

```sh
it2agent-inbox list                 # pending requests awaiting a human
it2agent-inbox show <id>            # one request + its policy decision & reason
it2agent-inbox approve <id> [--note …]
it2agent-inbox edit <id> [--action …] [--scope …] [--cost N] [--reversible y|n] [--note …]
it2agent-inbox reject <id> [--note …]
```
...
* `agent.inbox.requests` — one message per request; the broker message id
  **is** the request id (monotonic, unique).
* `agent.inbox.decisions` — one message per decision, body carries its
  `request_id`.
...
| `gate.py` | flags helper | the `agent.inbox` gate (fail-safe OFF). |
| `inbox.py` | glue | intake → policy → queue/attention → decision. |
| `it2agent_inbox.py` / `it2agent-inbox` | CLI | `submit`/`list`/`show`/`approve`/`edit`/`reject`. |

No module imports `iterm2`; attention goes through the `it2agent-emit`
subprocess. The broker is injected, never imported by the pure path.

## Run

```sh
# 1) enable the flag (or use --no-gate / IT2AGENT_FORCE=1)
it2agent-flag enable agent.inbox
# 2) start the broker (durable state lives there)
python3 it2agent/broker/it2agent_broker.py serve
# 3) submit / triage
it2agent/inbox/it2agent-inbox submit --action git.push --scope repo --session $ITERM_SESSION_ID --agent me
it2agent/inbox/it2agent-inbox list
it2agent/inbox/it2agent-inbox approve 1
```

## Tests

```sh
bash it2agent/inbox/tests/run_tests.sh
```
