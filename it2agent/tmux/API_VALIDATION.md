...
The Tier 1 daemon (`it2agent/daemon`) controls agent sessions through the
...
1. **Start the daemon with debug logging** so its `new_session` / `ingest` /
   ...
   - `agent.daemon`, `agent.status_board` flags ON (`it2agent-flag enable …`),
   - run `it2agent/daemon/it2agent_daemon.py` (see the daemon README).

2. **Spawn an agent under tmux -CC** from an iTerm2 tab:
   ```sh
   it2agent/tmux/it2agent-tmux spawn --no-gate --role probe --task api -- $SHELL -l
   ```
...
4. **Surface 5 — user vars:** in the tmux pane, run
   `it2agent-emit --no-gate role probe`. Then either watch the daemon re-snapshot
...
5. **Surface 2 — custom escape sequence:** in the tmux pane, emit a it2agent
   envelope (e.g. via the agent's messaging path, or by hand:
   `printf '\033]1337;Custom=id=it2agent:%s\a' "$(printf '{"v":1}' | base64)"`).
   Confirm the daemon logged `ingest: … known_type=…`. ☐ pass ☐ fail
   - If it FAILS raw but PASSES wrapped in `\ePtmux;…\e\\`, record that the
     agent-side emit must wrap under tmux (a follow-up for `it2agent-emit`).
   Notes: ________________________________________________
...
8. **Persistence — the whole point:** with the agent mid-task, **quit iTerm2**
   ...
   `it2agent/tmux/it2agent-tmux attach --role probe --task api`. Confirm the
